...
## Layer 1 & 2: IndexedDataset and GPTDataset

These two layers — the `.bin/.idx` file pair (storage & mmap random access) and GPTDataset (three-index system for sequencing & shuffling) — are covered in detail in [W1D2/0-dataset.md](../W1D2/0-dataset.md). That document explains:

- How variable-length documents are packed with EOD tokens into a flat `.bin` file
- The `.idx` binary layout (sequence lengths, byte offsets, document boundaries)
- Memory-mapped access via `numpy.memmap` — O(1) random access without loading data into RAM
- The three-index system (`document_index`, `sample_index`, `shuffle_index`)
- Flexible sequence length — same `.bin` data, different slicing
- Cross-document attention prevention via `_get_ltor_masks_and_position_ids()`

> **MegatronBridge Config** (Layer 1 & 2)
